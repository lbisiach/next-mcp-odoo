...
# ---------------------------------------------------------------------------
# Factory — returns the right connection type based on config.api_protocol
# ---------------------------------------------------------------------------

from typing import Union  # noqa: E402 (appended to existing imports)


def get_connection(
    config: "OdooConfig",
    timeout: int = OdooConnection.DEFAULT_TIMEOUT,
    performance_manager: Optional["PerformanceManager"] = None,
) -> Union["OdooConnection", "OdooJson2Connection"]:
    """Return an OdooConnection or OdooJson2Connection based on config.api_protocol.
    ...
    Usage::
        ...
        conn.connect()
        conn.authenticate()
        # use conn.search_read(), conn.execute_kw(), …
    """
    if config.api_protocol == "json2":
        from .json2_connection import OdooJson2Connection
        return OdooJson2Connection(config, timeout, performance_manager)
    return OdooConnection(config, timeout, performance_manager)
